...
export async function deleteLead(id) {
  ...
  const arr = await readJson(LEADS);
  await writeJson(LEADS, arr.filter((l) => l.id !== id));
}
...
export async function listReviews(limit = 300) {
  const db = getDb();
  if (db) {
    const snap = await db.collection("reviews").orderBy("createdAt", "desc").limit(limit).get();
    return snap.docs.map((d) => ({ ...d.data(), id: d.id }));
  }
  return (await readJson(REVIEWS)).slice().reverse();
}

export async function saveReview(r) {
  const record = { ...r, createdAt: new Date().toISOString() };
  const db = getDb();
  if (db) {
    const ref = await db.collection("reviews").add(record);
    return { ...record, id: ref.id };
  }
  const arr = await readJson(REVIEWS);
  const withId = { ...record, id: `rv_${Date.now().toString(36)}${Math.floor(Math.random() * 1e3)}` };
  arr.push(withId);
  await writeJson(REVIEWS, arr);
  return withId;
}
...
export async function deleteReview(id) {
  ...
  const arr = await readJson(REVIEWS);
  await writeJson(REVIEWS, arr.filter((x) => x.id !== id));
}
